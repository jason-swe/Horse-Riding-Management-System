...
function JockeyInspection() {
  ...
  return <RefereeLayout title="Jockey Eligibility" eyebrow={`Assignment context | ${race.name}`} description="The backend has no dedicated referee jockey-inspection persistence contract. Assignment data is read only." actions={<Link className="admin-header__button admin-header__button--ghost" to={`/referee/races/${raceId}`}>Race Detail</Link>}><section className="admin-live-state">Saving a jockey inspection is unavailable until a backend endpoint is defined.</section><section className="admin-panel"><div className="admin-data-table__wrap"><table className="admin-data-table"><thead><tr><th>Horse</th><th>Jockey</th><th>License</th><th>Assignment status</th></tr></thead><tbody>{race.participants.map((participant) => <tr key={participant.horseId}><td>{participant.horseName}</td><td>{participant.jockeyName}</td><td>{participant.jockeyLicense}</td><td>{formatStatus(participant.assignmentStatus)}</td></tr>)}</tbody></table></div></section></RefereeLayout>;
}
