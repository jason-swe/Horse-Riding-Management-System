...
function RaceMonitor() {
  ...
  return <RefereeLayout title="Race Monitor" eyebrow={`During race | ${race.name}`} description="Record backend-persisted incidents. Race lifecycle controls and local timers are intentionally unavailable." actions={<Link className="admin-header__button admin-header__button--ghost" to={`/referee/races/${raceId}`}>Race Detail</Link>}>
    ...
    <section className="admin-panel"><div className="admin-panel__header"><p className="admin-panel__eyebrow">Lifecycle contract</p><h2>Authoritative race controls unavailable</h2></div><p>Start, pause, stop, finish, and synchronized timer controls are not exposed by the backend. This page will not change race status locally.</p></section>
